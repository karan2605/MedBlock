...
contract('Create Account', () => {
  it("fetch add", async () => {
    ...
    const fetched_addr = await instance.returnNhsToAddr(nhsNumber).call(account_one); // return the blockchain address
    const fetched_cid = await instance.getHashByAddr(fetched_addr).call(account_one); // get the IPFS hash using blockchain address
